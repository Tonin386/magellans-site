...
for (let i = 0; i < numFlakes; i++) {
    ...
    snowFlake.style.left = `${Math.random() * 99}vw`;  // Position aléatoire sur l'axe X
    snowFlake.style.animationDuration = `${Math.random() * 3 + 7.5}s`;  // Durée aléatoire de la chute
    snowFlake.style.animationDelay = `${Math.random() * 10}s`;  // Retard aléatoire avant de commencer la chute
